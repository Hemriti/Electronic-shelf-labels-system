...
#define SCAN_TIMEOUT            0x0064  // ~4 seconds
...
static void send_hello(void * p_context) {
    if (!nus_ready) return;
    static const char msg[] = "hello from nrf ! \n";
...
static void ble_evt_handler(ble_evt_t * p_ble_evt) {
    switch (p_ble_evt->header.evt_id) {
        case BLE_GAP_EVT_ADV_REPORT: {
            ...
            if (is_uuid_present(&m_target_uuid, adv)) {
                sd_ble_gap_connect(&adv->peer_addr, &m_scan_params, &m_connection_param);
            }
            break;
        }
...
int main(void) {
    ...
    sd_ble_uuid_vs_add(&nus_base_uuid, &nus_uuid_type);
